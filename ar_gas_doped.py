...
def background_average_iter_func(data_x, data_y, total_n_events):
    ...
    for i in range(total_n_events):
        if (i == NEVENT):
            ...
            plt.xlabel(r"Sample Time ($\mu$s)", fontsize = 25)
...
df = pd.read_hdf('E:\Manchester_Uni\Physics_Year_4\MphysProject\Hecker\Calibration\data_alpha_gas_V1.h5', key='mydata')
...
plt.xlim(-50, 250)
plt.title("Event " + str(Nevent), fontsize = 25)
...
for i in range(10): 
    ...
    plt.plot(x_arr[starting_index_bin_0[correct_wf_arr[i]]], y_arr[correct_wf_arr[i]][starting_index_bin_0[correct_wf_arr[i]]], 'bo', label='Start')
    plt.plot(x_arr[ending_index_bin_0[correct_wf_arr[i]]], y_arr[correct_wf_arr[i]][ending_index_bin_0[correct_wf_arr[i]]], 'ro', label='End')
    plt.plot([np.min(x_arr), np.max(x_arr)], [std_arr_0[correct_wf_arr[i]], std_arr_0[correct_wf_arr[i]]], '--', color='red')
    plt.plot([np.min(x_arr), np.max(x_arr)], [0, 0], '--', color='black')
    plt.xlabel(r"Sample Time [$\mu$ s]", fontsize = 25)
    plt.ylabel("ADC Value", fontsize = 25)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)
    plt.title("Event " + str(correct_wf_arr[i]), fontsize = 25)
    plt.ylim(-1, 30)
    plt.legend(fontsize=25)
    plt.show()
...
plt.xlabel("ADC Value", fontsize = 25)
plt.ylabel("Counts", fontsize = 25)
plt.xticks(fontsize=22)
plt.yticks(fontsize=22)
plt.title("Total charge histogram", fontsize = 25)
plt.legend(fontsize=25)
plt.xlim(-100, 8000)
plt.show()
...
popt, pcov = curve_fit(gauss, xdata=bins_centres[8:45], ydata=n[8:45], p0=[70, 3000, 900, 3000])
print(popt)
