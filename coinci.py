...
df = pd.read_hdf('E:\Manchester_Uni\Physics_Year_4\MphysProject\Hecker\Calibration\calib.h5', key='mydata')
...
plt.plot(x_arr, y_arr[4], label='No bins')
plt.plot(x_arr[starting_index_bin_0[4]], y_arr[4][starting_index_bin_0[4]], 'bo', label='Start')
plt.plot(x_arr[ending_index_bin_0[4]], y_arr[4][ending_index_bin_0[4]], 'ro', label='End')
plt.plot([np.min(x_arr), np.max(x_arr)], [std_arr_0[4], std_arr_0[4]], '--', color='red')
plt.plot([np.min(x_arr), np.max(x_arr)], [0, 0], '--', color='black')
plt.xlabel("Sample Time (ns)", fontsize = 17)
plt.ylabel("ADC Value", fontsize = 17)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.title("Event " + str(4), fontsize = 22)
plt.legend()
plt.show()

# for i in range(7):
#     plt.plot(x_arr, y_arr[i], label='No bins')
#     plt.plot(x_arr[starting_index_bin_0[i]], y_arr[i][starting_index_bin_0[i]], 'bo', label='Start')
#     plt.plot(x_arr[ending_index_bin_0[i]], y_arr[i][ending_index_bin_0[i]], 'ro', label='End')
#     plt.plot([np.min(x_arr), np.max(x_arr)], [std_arr_0[i], std_arr_0[i]], '--', color='red')
#     plt.plot([np.min(x_arr), np.max(x_arr)], [0, 0], '--', color='black')
#     plt.xlabel("Sample Time (ns)", fontsize = 17)
#     plt.ylabel("ADC Value", fontsize = 17)
#     plt.xticks(fontsize=16)
#     plt.yticks(fontsize=16)
#     plt.title("Event " + str(i), fontsize = 22)
#     plt.legend()
#     plt.show()
...
def quadratic_func(a, b, c, d, x):
    return (a * (x + d)**2 + (x + d) * b)/1000000 + c

def func_to_mini(vars, x, y):
    a, b, c, d = vars
    return st.mode(y - quadratic_func(a, b, c, d, x)).mode[0]

guess = np.array([-0.0001,0.2,0.05,-x_arr[starting_index_bin_0[4]]])
answer = least_squares(func_to_mini, guess, args=(x_arr, y_arr[4]))
print(answer.x)

a, b, c, d = answer.x

plt.plot(x_arr, y_arr[4])
plt.plot(x_arr, quadratic_func(a, b, c, d, x_arr))
plt.show()
...
popt, pcov = curve_fit(gauss, xdata=bins_centres[5:30], ydata=n[5:30], p0=[1000, 5000, 460, 3000])

perr = np.sqrt(np.diag(pcov))


xspace = np.linspace(200, 2000, 100000)
