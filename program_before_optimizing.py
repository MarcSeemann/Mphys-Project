...
@jit
def depth_down(signal_arr):
    return np.min(signal_arr)


@jit
def depth_up(signal_arr):
    return np.max(signal_arr)



@jit
def fwhm_func(signal_arr):
    return
...
@jit
def all_data_min_x(nested_arr):
    general_arr = []
    for i in range(branch_len):
        general_arr.append(min_func(time_arr, nested_arr[i]))
    general_arr = np.array(general_arr)
    return general_arr


@jit
def all_data_max_x(nested_arr):
    general_arr = []
    for i in range(branch_len):
        general_arr.append(max_func(time_arr, nested_arr[i]))
    general_arr = np.array(general_arr)
    return general_arr


@jit
def all_data_fwhm(nested_arr):
    general_arr = []
    for i in range(branch_len):
        general_arr.append(FWHM(time_arr, nested_arr[i]))
    general_arr = np.array(general_arr)
    return general_arr

@jit
def all_data_depth_up(nested_arr):
    general_arr = []
    for i in range(branch_len):
        general_arr.append(depth_up(nested_arr[i]))
    general_arr = np.array(general_arr)
    return general_arr


@jit
def all_data_depth_down(nested_arr):
    general_arr = []
    for i in range(branch_len):
        general_arr.append(depth_down(nested_arr[i]))
    general_arr = np.array(general_arr)
    return general_arr
...
def peak_finding_func(data_x, data_y):
    ...
    if (index_var == -1):
        ...
        if peak_index_var >= 149:
            global delete_var
            delete_var = 1
            signal_index_arr = np.arange(low_x_idx, 149)
...
def background_average_iter_func(data_x, data_y):
    ...
    for i in range(total_n_events):
        if (i == Nevent):
            ...
            if delete_var == 1:
                delete_arr.append(i)
            plt.plot(x[signal_arr_indicies], y[signal_arr_indicies], color='orange')
            plt.show()
...
def bin_pos_func(x_min, x_max, n_bins, p_per_bin):
    bin_loc_arr = []
    def bin_pos(min, max, p_per_bin, iter):
        bin_width = p_per_bin * 2
        return (((bin_width) * (0.5 + iter)) + min)
    for i in range(n_bins):
        bin_loc_arr = np.append(bin_loc_arr, bin_pos(x_min, x_max, p_per_bin, i))
    return bin_loc_arr


def number_of_bins_func(x, p_per_bin):
    while(True):
        if (len(x) % p_per_bin) == 0:
            number_of_bins = int(len(x)/p_per_bin)
